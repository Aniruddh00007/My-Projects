...
// const { deleteTodo, updateTodo, toggleComplete } = useTodo();
  ...
  const { todos, handleDelete, handleUpdate } = useContext(TodoContextt);// Backend handlers
  ...
  const edit = () => {
    handleUpdate(todo.id, { title, content: msg }); // Backend update
    setIsTodoEditable(false);
  };

  const toggleCompleted = () => {
    handleToggle(todo.id); // Backend toggle
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <button
         ...
         // onClick={() => deleteTodo(todo.id)}
            onClick={() => handleDelete(todo.id)} // Backend delete
        >
          ❌
        </button>
      </div>
